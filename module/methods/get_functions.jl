...
function get_frequencies(
    ...
    return [ proportions(pop.strategies[ pop.membership .== g ],s:s) |> first for g in pop.all_groups, s in pop.all_strategies ]
end
...
function get_cooperation(
    ...
    return [ sum(pop.actions[ pop.membership .== g1, pop.membership .== g2]) / (pop.group_sizes[g1] * pop.group_sizes[g2] * pop.N * pop.N) for g1 in pop.all_groups, g2 in pop.all_groups ]
end
...
function get_avg_fitness(
    ...
    return [ mean(pop.fitness[ (pop.membership .== g) .& (pop.strategies .== s) ]) for g in pop.all_groups, s in pop.all_strategies ]
end
...
function get_reps_ind(
    ...
    return [ mean(pop.reps_ind[ pop.membership .== g1, pop.membership .== g2 ]) for g1 in pop.all_groups, g2 in pop.all_groups ]
end
